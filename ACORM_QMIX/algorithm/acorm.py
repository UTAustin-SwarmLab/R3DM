...
class ACORM_Agent(object):
    ...
    def train(self, replay_buffer, total_steps):
        ...
        if self.train_recl_freq !=0 and self.train_step % self.train_recl_freq == 0:
            ...
            metrics['train/recl_loss'] = recl_loss.item()
        else:
            metrics['train/recl_loss'] = 0.0
        loss = self.update_qmix(inputs, batch_o, batch_s, batch_r, batch_a, batch_last_a, batch_avail_a_n, batch_active, batch_dw, max_episode_len)
        ...
        metrics['train/batch_reward'] = batch_r.squeeze().sum(dim=-1).mean()
        return metrics
