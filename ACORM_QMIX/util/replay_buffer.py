...
class ReplayBuffer:
    def __init__(self, args, buffer_size):
        ...
        self.obs_text_dim = args.obs_text_dim
        ...
        if 'text' in args.algorithm or 'TEXT' in args.algorithm:
            self.buffer['obs_n_text'] = np.zeros([self.buffer_size, self.episode_limit + 1, self.N, self.obs_text_dim],np.float32)
        self.episode_len = np.zeros(self.buffer_size)

    def store_transition(self, episode_step, obs_n, s, avail_a_n, last_onehot_a_n, a_n, r, dw, obs_n_text=None):
        ...
        if obs_n_text is not None and 'obs_n_text' in self.buffer:
            self.buffer['obs_n_text'][self.episode_num][episode_step] = obs_n_text
        
    def store_last_step(self, episode_step, obs_n, s, avail_a_n, last_onehot_a_n, obs_n_text=None):
        self.buffer['obs_n'][self.episode_num][episode_step] = obs_n
        self.buffer['s'][self.episode_num][episode_step] = s
        self.buffer['avail_a_n'][self.episode_num][episode_step] = avail_a_n
    ...
    def store_obs_n_text(self, obs_n_text):
        if obs_n_text is None and 'obs_n_text' in self.buffer:
            raise Exception("obs_n_text is None")
        elif obs_n_text is not None and 'obs_n_text' in self.buffer:
            self.buffer['obs_n_text'][self.episode_num][:obs_n_text.shape[0]] = obs_n_text
    ...
    def sample(self, batch_size):
        ...
        for key in self.buffer.keys():
            if key == 'obs_n' or key == 's' or key == 'avail_a_n' or key == 'last_onehot_a_n' or key == 'obs_n_text':
