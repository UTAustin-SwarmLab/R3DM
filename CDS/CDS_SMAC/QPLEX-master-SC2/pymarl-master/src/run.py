...
def run(_run, _config, _log):
    ...
    # unique_token = "{}_{}_{}_{}".format(args.env_args.map_name, args.env_args.seed, args.name, datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))
    unique_token = "seed{}_{}".format(args.env_args['seed'], datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S"))

    args.unique_token = unique_token
    if args.use_tensorboard:
        tb_logs_direc = os.path.join(dirname(dirname(abspath(__file__))), "results", "tb_logs", f"{args.env_args['map_name']}/n{args.env_args['capability_config']['n_units']}")
...
def run_sequential(args, logger):
    ...
    args.unit_dim = 4 + runner.env.shield_bits_ally + runner.env.unit_type_bits
    ...
    env_name = args.env
    if env_name == 'sc2' or env_name == 'sc2v2':
        ...
        if (args.env == 'matrix_game_1' or args.env == 'matrix_game_2' or args.env == 'matrix_game_3') and \
            ...
            for i in range(args.n_actions):
                for j in range(args.n_actions):
                    ...
                    episode_sample = episode_sample[:, :max_ep_t]
                    episode_sample['actions'][0, :, :, 0] = new_actions
                    new_actions_onehot = th.zeros(episode_sample['actions'].squeeze(3).shape + (args.n_actions,)).to(args.device)
                    new_actions_onehot = new_actions_onehot.scatter_(3, episode_sample['actions'].to(args.device), 1)
                    episode_sample['actions_onehot'][:] = new_actions_onehot
                    if i == 0 and j == 0:
                        rew = th.Tensor([
                            8,
